...
query <- "UPDATE whse_sp.fwa_lakes_poly SET riparian_buffer_width_m = CASE 
	WHEN riparian_class = 'L1A' THEN 105 -- 0
	WHEN riparian_class = 'L1B' THEN 105 -- 10
	WHEN riparian_class = 'L2'  THEN 55 -- 15
	WHEN riparian_class = 'L3'  THEN 55 -- 7.5
	WHEN riparian_class = 'L4'  THEN 55 - 7.5
	END;"
run_sql_r(query, conn_list)
...
query <- "UPDATE whse_sp.fwa_rivers_poly SET riparian_buffer_width_m = CASE 
	...
	WHEN riparian_class = 'S3'  THEN 35
	WHEN riparian_class = 'S4'  THEN 30 -- 7.5
	WHEN riparian_class = 'S5'  THEN 30 -- 7.5
	WHEN riparian_class = 'S6'  THEN 30 -- 1
	END;"
run_sql_r(query, conn_list)
...
query <- "UPDATE whse_sp.fwa_wetlands_poly SET riparian_buffer_width_m = CASE 
	WHEN riparian_class = 'W1'  THEN 55 -- 20
	WHEN riparian_class = 'W2'  THEN 55 -- 15
	WHEN riparian_class = 'W3'  THEN 55 -- 7.5
	WHEN riparian_class = 'W4'  THEN 30 -- 7.5
	WHEN riparian_class = 'W5'  THEN 55 -- 20
	END;"
run_sql_r(query, conn_list)
...
query <- "UPDATE whse_sp.modelled_habitat_potential SET riparian_buffer_width_m = CASE 
	...
	END;"
run_sql_r(query, conn_list)
query <- "UPDATE whse_sp.modelled_habitat_potential SET riparian_buffer_geom = ST_Buffer(geom, riparian_buffer_width_m) WHERE riparian_buffer_width_m IS NOT NULL and tsa = 22;"
run_sql_r(query, conn_list)
...
query <- glue("CREATE TABLE whse_sp.riparian_buffers_okanagan (
...
join 
(select * from whse_sp.tsa_boundaries_2020 where tsa IN (22)) tsa -- (23, 11, 22, 18)
ON ST_Intersects(grd.geom, tsa.geom)"
grid_df <- sql_to_df(query, conn_list)

query <- "INSERT INTO whse_sp.riparian_buffers_okanagan ({grid_loop_fld}, geom)
