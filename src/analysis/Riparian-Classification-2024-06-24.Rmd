...
query <- "UPDATE whse_sp.fwa_lakes_poly SET riparian_buffer_width_m = CASE 
	WHEN riparian_class = 'L1A' THEN 0  -- 105 -- 0
	WHEN riparian_class = 'L1B' THEN 10 -- 105 -- 10
	WHEN riparian_class = 'L2'  THEN 10 -- 55 -- 15
	WHEN riparian_class = 'L3'  THEN 0  -- 55 -- 7.5
	WHEN riparian_class = 'L4'  THEN 0  -- 55 - 7.5
	END;"
run_sql_r(query, conn_list)
...
query <- "UPDATE whse_sp.fwa_rivers_poly SET riparian_buffer_width_m = CASE 
	WHEN riparian_class = 'S1A' THEN 55 -- 50
	WHEN riparian_class = 'S1B' THEN 55 -- 60
	WHEN riparian_class = 'S2'  THEN 35 -- 40
	WHEN riparian_class = 'S3'  THEN 25 -- 30
	WHEN riparian_class = 'S4'  THEN 4 -- 30 -- 7.5
	WHEN riparian_class = 'S5'  THEN 4 -- 30 -- 7.5
	WHEN riparian_class = 'S6'  THEN 0 -- 30 -- 1
	END;"
run_sql_r(query, conn_list)
...
query <- "UPDATE whse_sp.fwa_wetlands_poly SET riparian_buffer_width_m = CASE 
	WHEN riparian_class = 'W1'  THEN 15 -- 55 -- 20
	WHEN riparian_class = 'W2'  THEN 12 -- 55 -- 15
	WHEN riparian_class = 'W3'  THEN 3 -- 55 -- 7.5
	WHEN riparian_class = 'W4'  THEN 3 -- 30 -- 7.5
	WHEN riparian_class = 'W5'  THEN 15 -- 55 -- 20
	END;"
run_sql_r(query, conn_list)
...
query <- "UPDATE whse_sp.modelled_habitat_potential SET riparian_buffer_width_m = CASE 
	...
	WHERE tsa = 11;"
run_sql_r(query, conn_list)
query <- "UPDATE whse_sp.modelled_habitat_potential SET riparian_buffer_geom = ST_Buffer(geom, riparian_buffer_width_m) WHERE riparian_buffer_width_m IS NOT NULL and tsa = 11;"
run_sql_r(query, conn_list)
...
query <- glue("CREATE TABLE whse_sp.riparian_buffers_kamloops (
...
grd.map_tile
from 
whse_sp.nts_50k_grid grd
join 
(select * from whse_sp.tsa_boundaries_2020 where tsa IN (11)) tsa -- (23, 11, 22, 18)
ON ST_Intersects(grd.geom, tsa.geom)"
grid_df <- sql_to_df(query, conn_list)

query <- "INSERT INTO whse_sp.riparian_buffers_kamloops ({grid_loop_fld}, geom)
...
```{r eval = FALSE}
## rivers
system("ogr2ogr -overwrite -f \"FileGDB\" C:\\projects\\THLB_Proxy\\data\\output\\riparian_classes_tsa_23_22_18_11.gdb PG:\"dbname='prov_data' host='localhost' user='postgres'\" -nlt MULTIPOLYGON -nln fwa_rivers_poly -sql \"SELECT waterbody_poly_id::int, watershed_group_id::int, waterbody_type, gnis_name_1, fwa_watershed_code, local_watershed_code, watershed_group_code, left_right_tributary, feature_area_sqm, feature_length_m, riv.geom, riparian_class_reason, riparian_class, riparian_data_source FROM whse_sp.fwa_rivers_poly riv JOIN (select ST_Union(geom) as geom from whse_sp.tsa_boundaries_2020 where tsa IN (23, 22, 18, 11)) tsa ON ST_Intersects(tsa.geom, riv.geom) WHERE riparian_class IS NOT NULL\"")

## lakes
system("ogr2ogr -f \"FileGDB\" C:\\projects\\THLB_Proxy\\data\\output\\riparian_classes_tsa_23_22_18_11.gdb PG:\"dbname='prov_data' host='localhost' user='postgres'\" -nlt MULTIPOLYGON -nln fwa_lakes_poly -sql \"SELECT waterbody_poly_id::int, watershed_group_id::int, waterbody_type, gnis_name_1, fwa_watershed_code, local_watershed_code, watershed_group_code, left_right_tributary, feature_area_sqm, feature_length_m, lake.geom, riparian_class_reason, riparian_class, riparian_data_source FROM whse_sp.fwa_lakes_poly lake JOIN (select ST_Union(geom) as geom from whse_sp.tsa_boundaries_2020 where tsa IN (23, 22, 18, 11)) tsa ON ST_Intersects(tsa.geom, lake.geom) WHERE riparian_class IS NOT NULL\" -update")

## wetlands
system("ogr2ogr -f \"FileGDB\" C:\\projects\\THLB_Proxy\\data\\output\\riparian_classes_tsa_23_22_18_11.gdb PG:\"dbname='prov_data' host='localhost' user='postgres'\" -nlt MULTIPOLYGON -nln fwa_wetlands_poly -sql \"SELECT waterbody_poly_id::int, watershed_group_id::int, waterbody_type, gnis_name_1, fwa_watershed_code, local_watershed_code, watershed_group_code, left_right_tributary, feature_area_sqm, feature_length_m, wet.geom, riparian_class_reason, riparian_class, riparian_data_source FROM whse_sp.fwa_wetlands_poly wet JOIN (select ST_Union(geom) as geom from whse_sp.tsa_boundaries_2020 where tsa IN (23, 22, 18, 11)) tsa ON ST_Intersects(tsa.geom, wet.geom) WHERE riparian_class IS NOT NULL\" -update")

## streams
system("ogr2ogr -f \"FileGDB\" C:\\projects\\THLB_Proxy\\data\\output\\riparian_classes_tsa_23_22_18_11.gdb PG:\"dbname='prov_data' host='localhost' user='postgres'\" -nlt MULTILINESTRING -nln fwa_stream_networks_sp_modelled_habitat_potential -sql \"SELECT fid, linear_feature_id::int, fish_habitat_id, blue_line_key, watershed_key, gnis_name, fwa_watershed_code, watershed_group_code, downstream_route_measure, waterbody_key, fwa_fcode_label, observation_id, gradient_barrier_030_id,gradient_barrier_050_id,gradient_barrier_080_id,gradient_barrier_150_id,gradient_barrier_220_id,gradient_barrier_300_id,intermittent_id,fish_obstacle_point_id,obstruction_id,subsurfaceflow_id,fish_habitat,slope,slope_class,stream_order,stream_magnitude,stream.geom,channel_width,channel_width_source,community_watershed,riparian_class,riparian_class_reason,riparian_data_source FROM whse_sp.modelled_habitat_potential stream JOIN (select ST_Union(geom) as geom from whse_sp.tsa_boundaries_2020 where tsa IN (23, 22, 18, 11)) tsa ON ST_Intersects(tsa.geom, stream.geom) WHERE NOT inside_fwa_polygon AND riparian_class IS NOT NULL\" -update")
...
spatial_query <- "SELECT 
    ST_Intersection(grd.geom, buf.geom) as
  	FROM 
    public.kamloops_fdp_diff_11 buf
	CROSS JOIN 
	(SELECT * FROM {grid_table} WHERE {grid_loop_fld} = '{grid_row}') grd"
...
Layer: whse_sp.kamloops_fdp_diff_11
...
linear_weight(template_tif           = "data\\input\\bc_01ha_gr_skey.tif",  ## "S:\\FOR\\VIC\\HTS\\ANA\\workarea\\PROVINCIAL\\bc_01ha_gr_skey.tif"
			...
			grid_geom_fld            = "geom",
			dst_schema               = "whse",
			dst_tbl                  = "bc_riparian_buffers_remaining_bw_fdp_and_kamloops",
